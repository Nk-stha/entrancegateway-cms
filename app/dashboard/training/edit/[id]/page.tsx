...
export default function EditTrainingPage() {
  ...
  const { formData, errors, isSubmitting, handleChange, handleSubmit, resetForm, setFormData } = useTrainingForm();
  ...
  const handleUpdateTraining = async (data: TrainingFormData) => {
    const result = await trainingService.updateTraining(trainingId, data);
    ...
    if (result.success) {
      ...
      if (result.fieldErrors) {
        ...
        toast.error(result.error || 'Failed to update training. Please try again.');
      }
    }
  };
        ...
        <DashboardLayout>
          <div className="flex items-center justify-center py-12">
            <div className="text-center">
              ...
              <p className="text-gray-500 mb-4">Training not found</p>
              <button
    ...
    <ProtectedRoute>
      <DashboardLayout>
        ...
        <nav className="flex mb-6 text-sm text-gray-500 font-medium">
          ...
          <span className="mx-2 text-gray-300">/</span>
          <span style={{ color: 'var(--color-brand-navy)' }}>Edit Training</span>
        </nav>
        ...
        <div className="flex flex-col gap-4 mb-6 md:mb-8">
          <div>
            ...
            <p className="text-sm md:text-base text-gray-500 mt-1">
              Update training program details and configuration.
            </p>
          </div>
        ...
        <form
          id="training-form"
          onSubmit={(e) => handleSubmit(e, handleUpdateTraining)}
            ...
            <div className="space-y-4 md:space-y-6">
                  ...
                  {(formData.links || []).map((link, index) => (
                    <div key={index} className="flex gap-2">
                      <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-2">
                        <div>
                          <label htmlFor={`edit-link-label-${index}`} className="sr-only">
                            Link Label {index + 1}
                          </label>
                          <input
                            id={`edit-link-label-${index}`}
                            type="text"
                        ...
                        <div>
                          <label htmlFor={`edit-link-url-${index}`} className="sr-only">
                            Link URL {index + 1}
                          </label>
                          <input
                        ...
                        <div>
                          <label htmlFor={`edit-link-type-${index}`} className="sr-only">
                            Link Type {index + 1}
                          </label>
                          <select
                            id={`edit-link-type-${index}`}
                            value={link.linkType}
                            onChange={(e) => {
                              const newLinks = (formData.links || []).map((l, i) =>
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mb-2">
                  Upload a new file to replace existing materials
                </p>
