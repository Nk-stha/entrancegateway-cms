...
export default function CreateQuizTemplatePage() {
  ...
  const handleCreateTemplate = async (payload: CreateQuizTemplateRequest) => {
    try {
      const result = await quizTemplateService.createQuizTemplate(payload);

      if (result.success) {
        toast.success('Quiz template created successfully!', {
          description: `Template "${result.data?.name}" has been created.`,
        });
        ...
        toast.error(result.error || 'Failed to create quiz template');
      }

      return result;
    } catch {
      const message = 'An unexpected error occurred while creating the quiz template.';
      toast.error(message);
  ...
  return (
    <ProtectedRoute>
      <DashboardLayout>
        ...
        <div className="flex flex-col gap-4 mb-6 md:mb-8">
          <div>
            ...
            <p className="text-sm md:text-base text-gray-500 mt-1">
              Configure a new quiz template with questions, marks, and distribution settings.
            </p>
          </div>
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
            ...
            <button
              onClick={() => {
                const form = document.getElementById('quiz-template-form') as HTMLFormElement;
                form?.requestSubmit();
              }}
        ...
        {/* Form */}
        <form
          id="quiz-template-form"
          onSubmit={(e) => handleSubmit(e, handleCreateTemplate)}
